...
app.use('/ext/getlasttxs/:min', async function(req,res){
  db.get_last_txs(req.query.length, req.query.start, req.params.min * 100000000, function(txs){
	  lib.get_blockcount(function(blockcount) {
	  	txs = Object.keys(txs).map((key) => [txs[key]['blockindex'],txs[key]['blockhash'],txs[key]['txid'],txs[key]['vout'].length,txs[key]['total'],txs[key]['timestamp']]);
	  	res.send({data: txs, draw: req.query.draw, recordsTotal: blockcount, recordsFiltered: blockcount});
	  });
  });
});
